...
class InferenceAPI:
    ...
    def __init__(self) -> None:
        ...
        checkpoint = "/gris/gris-f/homestud/ssivakum/SASVi/src/sam2/checkpoints/sam2.1_hiera_large.pt"
    ...
    def start_session(self, request: StartSessionRequest, offload_video_flag: bool = False) -> StartSessionResponse:
        with self.autocast_context(), self.inference_lock:
            ...
            offload_video_to_cpu = self.device.type == "mps" or offload_video_flag
    ...
    def add_points(
        self, request: AddPointsRequest, normalize_coords: bool = False
    ) -> PropagateDataResponse:
        with self.autocast_context(), self.inference_lock:
    ...
    def __get_mask_for_object(
        ...
        return PropagateDataValue(
            object_id=object_id,
            mask=MaskArray(
                array=np.array(mask, dtype=np.bool)
            ),
        )
