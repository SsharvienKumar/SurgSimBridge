...
@export var incknife_scale := 10
@export var incknife_position_offset := Vector3(-1.5, -1.0, 14.4) # match where incision knife starts
...
func update_incknife(incknife: Dictionary, video_width: int, video_height: int):
	...
	if incknife.has("tip"):
		...
		position.x = norm_tx * incknife_scale + incknife_position_offset.x #camera.camera_position_offset.x#
		position.y = norm_ty * incknife_scale + incknife_position_offset.y #camera.camera_position_offset.y#
